...
public class UtilidadesEmpresa {
    ...
    public static List<Empleado> getEmpleadosPymePracticas(List<Empresa> empresas) {
        List<Empleado> empleadosPracticas = new ArrayList<>();

        for (Empresa empresa : empresas) {
            if ("PYME".equalsIgnoreCase(String.valueOf(empresa.getTipoEmpresa()))) {
                for (Empleado empleado : empresa.getEmpleado()) {
                    if ("PRACTICAS".equalsIgnoreCase(String.valueOf(empleado.getContrato()))) {
                        empleadosPracticas.add(empleado);
                    }
                }
            }
        }
        return empleadosPracticas;
    }
}
